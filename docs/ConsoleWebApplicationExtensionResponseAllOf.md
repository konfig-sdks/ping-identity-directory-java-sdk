...
|**schemas** | **List&lt;EnumconsoleWebApplicationExtensionSchemaUrn&gt;** |  |  [optional] |
...
|**baseContextPath** | **String** | Specifies the base context path that should be used by HTTP clients to reference content. The value must start with a forward slash and at least one additional character and must represent a valid HTTP context path. |  [optional] |
